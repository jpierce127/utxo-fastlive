// Base code it taken from: https://github.com/in3rsha/bitcoin-utxo-dump
// Bitcoin UTXO heat chart image is generate in two parts. First golang script collect the data and python script render the image
package main

import (
	"bufio" // bulk writing to file
	"bytes"
	"encoding/hex" // convert byte slice to hexadecimal
	"flag"
	"fmt"
	"github.com/jpierce127/utxo-fastlive/bitcoin/bech32"     // segwit bitcoin addresses
	"github.com/jpierce127/utxo-fastlive/bitcoin/btcleveldb" // chainstate leveldb decoding functions
	"github.com/jpierce127/utxo-fastlive/bitcoin/keys"       // bitcoin addresses
	"github.com/syndtr/goleveldb/leveldb"                    // go get github.com/syndtr/goleveldb/leveldb
	"github.com/syndtr/goleveldb/leveldb/opt"                // set no compression when opening leveldb
	"os"
	"os/exec"   // execute shell command (check bitcoin isn't running)
	"os/signal" // catch interrupt signals CTRL-C to close db connection safely
	"runtime"   // Check OS type for file-handler limitations
	"strconv"
	"strings" // parsing flags from command line
	"syscall" // catch kill commands too
	"time"
)
...
func main() {
	...
	if err == nil {
		...
		blockHeight, err := strconv.ParseUint(string(response), 10, 64)
		if err != nil {
			fmt.Println(err)
			panic("Unable to get Block Height. Stopping the execution.")
		}
		fmt.Println("Block Height is at ", blockHeight)

		// Stopping the BTC core/daemon
		fmt.Println("Bitcoin is running,  shutting down with `bitcoin-cli stop`. We don't want to access the chainstate LevelDB while Bitcoin is running.")
		cmd = exec.Command("bitcoin-cli", "stop")
		response, err := cmd.Output()
		if err != nil {
			...
			panic("Could not stop the Bitcoin core, please stop it manually.")
		} else if string(response) == "Bitcoin Core stopping" {
			fmt.Println(" The Bitcoin core is stopped! \nContinuing...")
		}
	...
	defaultFile := fmt.Sprintf("%d_utxo.csv", blockHeight)
	...
	fields := flag.String("f", "height,amount", "Fields to include in output. [count,txid,vout,height,amount,coinbase,nsize,script,type,address]")
	...
	// Final Progress Report - i = coin_count (in python)
	...
	if fieldsSelected["amount"] {
		// convert satoshis to BTC (float with 8 decimal places)
		fmt.Printf("Total BTC:   %.8f\n", float64(totalAmount)/float64(100000000))
	}
	...
	// Render the utxo-live chart (in python 3)
	fmt.Println("Rendering and saving image...")
	arguments := fmt.Sprintf("%d %d", blockHeight, i)
	fmt.Println(arguments)
	cmd = exec.Command("/usr/bin/python3  histogram.py ", arguments) // args  <block height> <coin_count>
	_, err = cmd.Output()
	if err != nil {
		panic(err)
		//  panic("Could not render the image!")
	}

	// Enable Bitcoin daemon
	fmt.Println("Starting the Bitcoin daemon.")
	cmd = exec.Command("bitcoind", "-daemon") //
	_, err = cmd.Output()
	if err != nil {
		panic("Could not starting the Bitcoin daemon!\nPlease start the daemon manually.")
	}
